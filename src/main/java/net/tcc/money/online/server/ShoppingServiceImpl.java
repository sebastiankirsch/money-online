...
public class ShoppingServiceImpl extends RemoteServiceServlet implements ShoppingService {
	...
	@Nonnull
	@Override
	public Iterable<Purchase> loadPurchases() {
		return loadAll(PersistentPurchase.class, toPurchase, "purchaseDate DESC");
	}
	...
	private <Dto, Persistence> Iterable<Dto> loadAll(Class<Persistence> persistenceClass,
			Function<Persistence, Dto> function, String order) {
	...
	public Category createCategory(@Nonnull final String name, @Nullable final Category parent) {
		final String dataSetId = getDataSetId();
        return ServerTools.executeWithoutTransaction(new PersistenceTemplate<Category>() {
            @Override
            public Category doWithPersistenceManager(PersistenceManager pm) {
                ...
                try {
                    return pm.makePersistent(new PersistentCategory(dataSetId, name, parentCategory)).toCategory();
                } finally {
                    tx.commit();
                }
            }
